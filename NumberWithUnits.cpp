...
const int a = 97, z = 122, A = 64, Z = 90;
const int char_ = 95, spec_chars_from = 43, spec_chars_to = 57;
map<string, map<string, double>> units_map; // Self implementation of units conversion table.

bool char_is_a_number(char c)
{
    return c >= spec_chars_from && c <= spec_chars_to; // (*). (+), (-), (/), (.) AND (0-9)
}
...
{
    return (c >= A && c <= Z) || c == char_; // 'A'-'Z' OR (_)
}
...
namespace ariel
{
    ...
    */
    ...
    {
        string helper;
        double conv_rate_mid_unit = 0, conversion_rate = 0;
        bool b = false;
        string unit1, unit2;
        for (const auto &p1 : units_map)
        {
            helper = p1.first;
            for (const auto &p2 : units_map)
            {
                unit1 = p2.first;

                for (const auto &p3 : units_map)
                {
                    unit2 = p3.first;
                    // if unit1 and helper are connected
                    b = (unit1 != unit2) && (unit2 != helper);
                    if (units_map[helper].count(unit1) == 1 && units_map[helper].count(unit2) == 1 && b)
                    {
                        conv_rate_mid_unit = units_map[unit1][helper]; // Get conversion rate

                        conversion_rate = conv_rate_mid_unit * (units_map[helper][unit2]);
                        units_map[unit1][unit2] = conversion_rate;     // Ex.  KM->CM
                        units_map[unit2][unit1] = 1 / conversion_rate; // AND  CM->KM
                    }
                }
            }
        }
    ...
    */
    void NumberWithUnits::read_units(ifstream &ifstream)
    {
        ...
        add_all_valid_units(l_unit, r_unit); // Iterate over l_unit's "neighbors" to
                                             // match with r_unit's.
    ...
    */
    ...
    NumberWithUnits operator+(const NumberWithUnits &n1, const NumberWithUnits &n2)
    {
        ...
        if (u1 != u2)
        { // if n1 and n2 are of same unit type
